...
function registerValidation(req, res, next) {
  ...
  if (!checkRequestBody(req)) 
    return res.status(responseStatus.BAD_REQUEST().status).send(commonMessages.validation.body.missing);
  if (!checkIfObjectExist(req.body.user))
    return res.status(responseStatus.BAD_REQUEST().status).send(commonMessages.validation.object.missing('user'));
  if (!checkObjectsKeys(req.body.user, userObjectKeys))
    return res.status(responseStatus.BAD_REQUEST().status).send(commonMessages.validation.object.invalid('user'));
